...
class GroupLights extends StatefulWidget {
  ...
  @override
  State<GroupLights> createState() => _GroupLightState();
}

class _GroupLightState extends State<GroupLights> {
    ...
    for (final deviceStr in savedDevices) {
      ...
      if (json['deviceId'] == widget.device.deviceId) {
        setState(() {
          ...
          // Correctly parse elements as List<String>
          final elementsStr = (json['elements'] ?? '') as String;
          _elements = elementsStr
              .split(',')
              .map((e) => e.trim())
  ...
  Future<void> _updateBrightness(double value) async {
    setState(() {
      _brightness = value.round().clamp(1, 255);
      _saveState();
    });
    ...
    for (String elementId in _elements) {
      String message = '#*2*$elementId*$_brightness*#';
                ...
                GestureDetector(
                  ...
                  child: Transform.translate(
                    ...
                    child: SvgPicture.asset(
                      'assets/icons/Card_Icons/ListGroupSingleLight.svg',
                      height: 50,
                      width: 50,
                    ),
                  ),
